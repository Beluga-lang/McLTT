...
Section nf_subsumption_dec.
  ...
  Ltac impl_obl_tac :=
    ...
    | |- ?j < ?i -> ~ (nf_subsumption _ _) =>
        let H := fresh "H" in
        let H0 := fresh "H" in
        intros H H0; dependent destruction H0; lia
    | A := _ : nf |- _ <> _ -> ~ nf_subsumption _ _ =>
        let H := fresh "H" in
        let H0 := fresh "H" in
        subst A; intros H H0; dependent destruction H0; congruence
    end.
...
Section lookup.
  ...
  #[tactic="impl_obl_tac"]
  Equations lookup G (HG : {{ ⊢ G }}) x : { A | {{ #x : A ∈ G }} /\ (exists i, {{ G ⊢ A : Type@i }}) } + { forall A, ~ {{ #x : A ∈ G }} } :=
  | {{{ G, A }}}, HG, x with x => {
...
Section pi_conversion.
  #[local]
  Ltac impl_obl_tac := idtac.

  #[tactic="impl_obl_tac"]
  Equations pi_conversion G i C (HC : {{ G ⊢ C : Type@i }}) : { '(A, B) | {{ G ⊢ Π A B ≈ C : Type@i }} } + { forall A B, ~{{ G ⊢ Π A B ≈ C : Type@i }} } :=
  | G, i, C, HC =>
      let (W, _) := nbe_impl G C  {{{ Type@i }}} _ in
      case_on W
  where
  case_on 
End pi_conversion.
...
Section type_check.
  ...
  | G, i, A, HA, M, H with M => {
    ...
    | {{{ rec M' return A' | zero -> MZ | succ -> MS end }}} =>
        ...
        pureb _
    | {{{ Π A' B' }}} =>
        let*o->b (exist _ j _) := min_level_infer G {{{ Π A' B' }}} _ while _ in
        let*b _ := typ_subsumption_dec G (max i (S j)) {{{ Type@j }}} _ A _ while _ in
        pureb _
    | {{{ λ A' M' }}} with nbe_impl G A {{{ Type@i }}} _ => {
      | exist _ n{{{ Π W1 W2 }}} _ => 
          let*o->b (exist _ j _) := min_level_infer G A' _ while _ in
          let*b _ := conversion_dec G {{{ Type@(max i j) }}} A' _ W1 _ while _ in
          let*b _ := type_check {{{ G, A' }}} i W2 _ M' _ while _ in
          pureb _
      | _ => left _
      }
    | {{{ M' N' }}} with min_type_infer G _ M' _ => {
      | inleft (exist _ D'j _) with (fst D'j) => {
        | {{{ Π B C }}} => pureb _
        | _ => left _
        }
      | inright HnotM' => _
  ...
  Next Obligation with mautosolve.
    ...
    assert (exists B', {{ G, A' ⊢ M' : B' }} /\ {{ G ⊢ Π A' B' ⊆ A }}) as [? []] by mauto using wf_fn_inversion...
  Qed.

  Next Obligation with mautosolve.
    progressive_inversion. eassumption.
    impl_obl_tac.
  Qed.

  Next Obligation with mautosolve.
    enough (exists i, {{ G, ℕ ⊢ A' : Type@i }}) as [] by firstorder.
