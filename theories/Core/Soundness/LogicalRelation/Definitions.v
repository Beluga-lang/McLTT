...
Definition nat_glu_pred i : glu_pred := fun Γ m M a => {{ Γ ⊢ M ® nat_typ_pred i }} /\ glu_nat Γ m a.
...
| mk_neut_glu_pred : forall Γ m M A c,
    {{ Γ ⊢ M ® neut_typ_pred i C }} ->
    {{ Dom c ≈ c ∈ per_bot }} ->
    (forall Δ σ V v, {{ Δ ⊢w σ : Γ }} ->
                {{ Rne C in length Δ ↘ V }} ->
                {{ Rne c in length Δ ↘ v }} ->
                {{ Δ ⊢ m[σ] ≈ v : M[σ] }}) ->
    {{ Γ ⊢ m : M ® ⇑ A c ∈ neut_glu_pred i C }}.
...
| mk_pi_typ_pred : forall Γ IT OT M,
    {{ Γ ⊢ M ≈ Π IT OT : Type@i }} ->
    {{ Γ , IT ⊢ OT : Type@i }} ->
    ...
    {{ Γ ⊢ M ® pi_typ_pred i IR IP IEl OP }}.
...
| mk_pi_glu_pred : forall Γ IT OT m M a,
    {{ Γ ⊢ m : M }} ->
    {{ Dom a ≈ a ∈ elem_rel }} ->
    {{ Γ ⊢ M ≈ Π IT OT : Type@i }} ->
    {{ Γ , IT ⊢ OT : Type@i }} ->
    ...
    {{ Γ ⊢ m : M ® a ∈ pi_glu_pred i IR IP IEl elem_rel OEl }}.
...
Definition glu_univ (i : nat) (A : domain) (B : domain) : typ_pred :=
  fun Γ M => exists P El, {{ Glu A ≈ B ∈ glu_univ_elem i ↘ P ↘ El }} /\ {{ Γ ⊢ M ® P }}.
Arguments glu_univ i A B Γ M/.
...
Section GluingInduction.
  ...
  | i, P, El, a, b, H =>
      glu_univ_elem_core_ind
        i
        (fun j lt_j_i A Γ M => glu_univ j A A Γ M)
        (motive i)
