...
Proof.
  ...
  assert {{ Γ ⊢s Wk∘(Wk∘IdMrecM) ≈ Wk∘(Id,,M) : Γ }} by mauto 4.
  assert {{ Γ ⊢s (Wk∘Wk)∘IdMrecM ≈ Id : Γ }} by mauto 4.
  assert {{ Γ ⊢ #1[IdMrecM] ≈ #0[Id,,M] : ℕ }} by mauto 3.
  assert {{ Γ ⊢ #1[IdMrecM] ≈ M : ℕ }} by mauto 4.
  assert {{ Γ ⊢ succ #1[IdMrecM] ≈ succ M : ℕ }} by mauto 2.
  assert {{ Γ ⊢ (succ #1)[IdMrecM] ≈ succ M : ℕ }} by mauto 4.
...
Proof.
  ...
  set (σM1 := {{{ σ,,M1[σ] }}}).
  set (σM1M2 := {{{ σM1,,M2[σ] }}}).
  set (σM1M2N := {{{ σM1M2,,N[σ] }}}).
  eapply wf_conv; mauto 3.
  transitivity {{{ B[IdM1M2N∘σ] }}}; [mauto 3 |].
  eapply exp_eq_sub_cong_typ2'; mauto 3.
  assert {{ Γ ⊢ (Eq A[Wk∘Wk] #1 #0)[σM1M2] : Type@i }} by mauto 4.
  assert {{ Δ ⊢ N : (Eq A[Wk∘Wk] #1 #0)[IdM1M2] }} by (eapply wf_conv; mauto 4).
  ...
  enough {{ Γ ⊢s σM1M2 ≈ IdM1M2∘σ : Δ, A, A[Wk] }} by (econstructor; mauto 3).
...
Proof.
  ...
  assert {{Γ ⊢ A[Wk∘Wk][σ,,M1[σ],,M2[σ]] ≈ A[σ] : Type@i}}
  ...
  assert {{ Γ ⊢ N[σ] : (Eq A[Wk∘Wk] #1 #0)[σ,,M1[σ],,M2[σ]] }}
  ...
  {
    transitivity {{{ Eq (A[Wk∘Wk][q (q σ)]) (#1[q (q σ)]) (#0[q (q σ)]) }}};
  ...
  assert {{ Γ, A[σ] ⊢ #0[q (q σ)∘(Id,,#0)] ≈ #0 : A[Wk∘Wk][q (q σ)∘(Id,,#0)] }}
  ...
  assert {{ Γ, A[σ] ⊢ #1[q (q σ)∘(Id,,#0)] ≈ #0 : A[Wk∘Wk][q (q σ) ∘ (Id,,#0)] }}
    by (eapply wf_exp_eq_conv; mauto 3).
  ...
  assert {{ Δ, A ⊢ A[Wk∘Wk][Id,,#0] : Type@i }} by (eapply exp_sub_typ; mauto 3).
  ...
  assert {{ Δ, A ⊢s Id,,#0,,refl A[Wk] #0 : Δ, A, A[Wk], Eq A[Wk∘Wk] #1 #0 }} by (econstructor; mauto 3).
  ...
  assert {{ Γ ⊢s Id,,M1[σ],,M2[σ],,N[σ] : Γ, A[σ], A[σ][Wk], Eq A[σ][Wk∘Wk] #1 #0}} by mauto 3.

  assert {{ Γ, A[σ], A[σ][Wk] ⊢ A[Wk][q σ∘Wk] ≈ A[σ][Wk∘Wk] : Type@i }} by mauto 3.

  assert {{ Γ, A[σ], A[σ][Wk] ⊢ #0 : A[Wk][q σ∘Wk] }} by (eapply wf_conv; mauto 3).

  assert {{ Γ, A[σ], A[σ][Wk], Eq A[σ][Wk∘Wk] #1 #0 ⊢s q (q σ)∘Wk : Δ,A,A[Wk] }} by mauto 3.
  ...
  assert {{ Γ, A[σ], A[σ][Wk], Eq A[σ][Wk∘Wk] #1 #0 ⊢ #0 : (Eq A[Wk∘Wk] #1 #0)[q (q σ)∘Wk] }}
  ...
  assert {{ Γ ⊢ A[Wk][(q σ∘Wk)∘(Id,,M1[σ],,M2[σ])] ≈ A[σ] : Type@i }}.
  {
    transitivity {{{ A[Wk][σ,,M1[σ]] }}};
      [eapply exp_eq_sub_cong_typ2' |]; mauto 3.
  }
  ...
  assert {{ Γ ⊢ (Eq A[Wk∘Wk] #1 #0)[σ,,M1[σ],,M2[σ]] ≈ Eq A[σ] M1[σ] M2[σ] : Type@i }}
    by (etransitivity; econstructor; mauto 3).
  assert {{ Γ ⊢ (Eq A[Wk∘Wk] #1 #0)[(q (q σ)∘Wk)∘(Id,,M1[σ],,M2[σ],,N[σ])] ≈ Eq A[σ] M1[σ] M2[σ] : Type@i }}.
  {
    transitivity {{{ (Eq A[Wk∘Wk] #1 #0)[σ,,M1[σ],,M2[σ]] }}};
      [eapply exp_eq_sub_cong_typ2' |]; mauto 3.
  }
...
Proof.
  ...
  assert {{ Γ, A, A[Wk] ⊢ Eq A[Wk∘Wk] # 1 # 0 : Type@i }} by (econstructor; mauto 3; eapply var_compose_subs; mauto 2).
  ...
  assert {{ Γ ⊢ N : (Eq A[Wk∘Wk] #1 #0)[Id,,M1,,M2]}}.
  {
    eapply wf_conv; mauto 2.
    symmetry; mauto 2.
  }
...
Proof.
  ...
  assert {{ Γ ⊢ refl A M : (Eq A[Wk∘Wk] #1 #0)[Id,,M,,M]}}.
  {
    eapply wf_conv; mauto 2.
    symmetry.
    mauto 3.
  ...
  assert {{ Γ ⊢ A[Wk∘Wk][Id,,M,,M] ≈ A : Type@i }} by mauto 3.
