...
Ltac unfold_functional_eval_prop := unfold functional_eval_exp_prop, functional_eval_natrec_prop, functional_eval_app_prop, functional_eval_sub_prop in *.

Lemma functional_eval_exp : forall {M p m1} (Heval1 : {{ ⟦ M ⟧ p ↘ m1 }}), functional_eval_exp_prop M p m1 Heval1
with functional_eval_natrec : forall {A MZ MS m p r1} (Heval1 : {{ rec m ⟦return A | zero -> MZ | succ -> MS end⟧ p ↘ r1 }}), functional_eval_natrec_prop A MZ MS m p r1 Heval1
with functional_eval_app : forall {m n r1} (Heval1 : {{ $| m & n |↘ r1 }}), functional_eval_app_prop m n r1 Heval1
with functional_eval_sub : forall {σ p p1} (Heval1 : {{ ⟦ σ ⟧s p ↘ p1 }}), functional_eval_sub_prop σ p p1 Heval1.
Proof with mauto.
  all: clear functional_eval_exp functional_eval_natrec functional_eval_app functional_eval_sub; intros.
  - (* functional_eval_exp *)
    induction Heval1
      ...
      unfold_functional_eval_prop; mauto;
